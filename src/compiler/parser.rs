use super::ast::{Program, TopLevel, Statement, Expression, BinaryOperator, UnaryOperator, DataType};
...
impl Parser {
    ...
    fn parse_top_level(&mut self) -> Result<TopLevel, String> {
        ...
        if self.match_token(Token::Interrupt) {
            ...
            self.consume(Token::Newline, "Expected newline after INTERRUPT definition")?;
    ...
    fn parse_type(&mut self) -> Result<DataType, String> {
        if self.match_token(Token::Byte) { return Ok(DataType::Byte); }
        if self.match_token(Token::Word) { return Ok(DataType::Word); }
        if self.match_token(Token::Bool) { return Ok(DataType::Bool); }
        Err(format!("Expected type (BYTE, WORD, BOOL), found {:?}", self.peek()))
    }
    ...
    fn parse_statement(&mut self) -> Result<Statement, String> {
        ...
        if self.match_token(Token::Return) {
            if self.check(Token::Newline) || self.check(Token::EOF) { // Basic Return
        ...
        if let Token::Identifier(name) = self.peek().clone() {
            ...
            return Err(format!("Unexpected token after identifier {}: {:?}", name, self.peek()));
        }
    ...
    fn check_block_end(&self) -> bool {
        let t = self.peek();
        matches!(t, Token::End | Token::Else | Token::Wend | Token::Next | Token::Loop)
    }
    ...
    fn parse_for(&mut self) -> Result<Statement, String> {
        ...
        Ok(Statement::For(var_name, start_expr, end_expr, step_expr, body))
    }
    ...
    fn parse_precedence(&mut self, precedence: Precedence) -> Result<Expression, String> {
        ...
        while precedence <= self.get_precedence(self.peek()) {
            ...
            let binary_op = self.token_to_binary_op(&op).ok_or("Expected binary operator")?;
...
mod tests {
    ...
    fn test_parse_if_statement() {
        ...
        let tokens = tokenize(input);
        let mut parser = Parser::new(tokens);
        let stmt = parser.parse_statement().expect("Failed to parse IF statement");
    ...
    fn test_parse_while_statement() {
        ...
        let tokens = tokenize(input);
        let mut parser = Parser::new(tokens);
        let stmt = parser.parse_statement().expect("Failed to parse WHILE statement");
    ...
    fn test_parse_for_statement() {
        ...
        let tokens = tokenize(input);
        let mut parser = Parser::new(tokens);
        let stmt = parser.parse_statement().expect("Failed to parse FOR statement");
