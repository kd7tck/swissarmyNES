...
pub async fn compile(body: String) -> impl IntoResponse {
    ...
    match result {
        ...
        Err(join_err) => {
            ...
            (StatusCode::INTERNAL_SERVER_ERROR, format!("Internal Server Error: {}", join_err)).into_response()
        }
    }
}
...
fn compile_source(source: &str) -> Result<Vec<u8>, String> {
    ...
    analyzer.analyze(&program).map_err(|e| format!("Analysis Error: {:?}", e))?;

    // 4. Codegen
    let symbol_table = analyzer.symbol_table;
    let mut codegen = CodeGenerator::new(symbol_table);
    let asm_lines = codegen.generate(&program).map_err(|e| format!("Codegen Error: {:?}", e))?;
    let asm_source = asm_lines.join("\n");

    // 5. Assembler
    let assembler = Assembler::new();
    let rom = assembler.assemble(&asm_source).map_err(|e| format!("Assembler Error: {:?}", e))?;
