...
mod tests {
    ...
    fn test_poke_word_variable() {
        ...
        // Verify Assembly
        // Look for assignment of $2006 to Ptr (at $0420)
        let found_assignment = asm_lines.iter().any(|line| line.contains("LDA #$06"))
            && asm_lines.iter().any(|line| line.contains("STA $0420"))
            && asm_lines.iter().any(|line| line.contains("LDA #$20"))
            && asm_lines.iter().any(|line| line.contains("STA $0421"));
        ...
        let found_ptr_load = asm_lines.iter().any(|line| line.contains("LDA $0420"))
            && asm_lines.iter().any(|line| line.contains("STA $02"))
            && asm_lines.iter().any(|line| line.contains("LDA $0421"))
        ...
        let found_assignment = asm_lines.iter().any(|line| line.contains("LDA #$07"))
            ...
            && ((asm_lines.iter().any(|line| line.contains("LDA #$20"))
                ...
                || (asm_lines.iter().any(|line| line.contains("LDX #$20"))
                    && asm_lines.iter().any(|line| line.contains("STX $0421"))));
