...
mod tests {
    ...
    fn test_read_string() {
        ...
        let found_hello = asm_lines.iter().any(|line| line.contains("db $48, $65, $6C, $6C, $6F, $00"));
        assert!(found_hello, "DATA 'Hello' with null terminator not found");

        // Verify READ call
        let found_call = asm_lines.iter().any(|line| line.contains("JSR Runtime_ReadString"));
        assert!(found_call, "JSR Runtime_ReadString not found");

        // Verify Runtime_ReadString implementation
        let found_impl = asm_lines.iter().any(|line| line.contains("Runtime_ReadString:"));
        assert!(found_impl, "Runtime_ReadString implementation not found");

        let found_loop = asm_lines.iter().any(|line| line.contains("Runtime_ReadString_Loop:"));
