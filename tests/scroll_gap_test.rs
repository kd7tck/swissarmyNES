...
mod tests {
    ...
    fn test_scroll_load_row_asm() {
        ...
        // Define 'arr'
        symbol_table.define("arr".to_string(), DataType::Array(Box::new(DataType::Byte), 32), swissarmynes::compiler::symbol_table::SymbolKind::Variable).unwrap();
        ...
        if let Some(start) = asm_str.find("Runtime_Scroll_LoadRow:") {
            if let Some(end_offset) = asm_str[start..].find("Scroll_LoadRow_Done:") {
                ...
                assert!(routine.contains("CMP #$F0"), "Runtime_Scroll_LoadRow missing CMP #$F0 check");
                assert!(routine.contains("Scroll_Row_Gap_Skip"), "Runtime_Scroll_LoadRow missing Gap Skip Label");
            } else {
    ...
    fn test_scroll_load_column_asm() {
        ...
        if let Some(start) = asm_str.find("Runtime_Scroll_LoadColumn:") {
            if let Some(end_offset) = asm_str[start..].find("Scroll_LoadColumn_Done:") {
                ...
                assert!(!routine.contains("CMP #$F0"), "Runtime_Scroll_LoadColumn SHOULD NOT check CMP #$F0");
            } else {
